...
export type GetPathParameter<Path extends string> = Path extends
  `${infer A}/${infer B}` ? IsPathParameter<A> | GetPathParameter<B>
  : IsPathParameter<Path>;

export type ResolvePath<Path extends string> = Prettify<
  & {
    [
      Param in GetPathParameter<Path> as Param extends `${string}?` ? never
        : Param
    ]: string;
  }
  & {
...
type IsBothObject<A, B> = A extends Record<string | number | symbol, unknown>
  ? B extends Record<string | number | symbol, unknown>
    ? IsClass<A> extends false ? IsClass<B> extends false ? true
      : false
    : false
  : false
  : false;
...
> = Schema extends undefined ? unknown
  : Schema extends StandardSchemaV1 ? StandardSchemaV1.InferOutput<Schema>
  : Schema extends ZodTypeAny ? z.infer<Schema>
  : Schema extends string
    ? Definitions extends Record<Schema, infer NamedSchema> ? NamedSchema
    : Definitions
  : unknown;
...
> {
  ...
  response: {} extends A["response"] ? {} extends B["response"] ? {}
    : B["response"]
...
> = (
  ...
  {} extends Route["response"] ? unknown
...
export type CoExist<Original, Target, With> = IsAny<Target> extends true
  ? Original
  : Original extends Record<string, unknown> ? {
      [K in keyof Original]: Original[K] extends Target ? Original[K] | With
        : Original[K];
    }
...
> = (
  ...
  | MaybePromiseIterable<
    {} extends Route["response"] ? unknown
      :
        | (Route["response"] extends { 200: any } ? Route["response"]
...
) => infer Returned ? (context: Context) => Returned | MaybePromise<void>
  : never;
...
export type CreateClient<
  Path extends string,
  Property extends Record<string, unknown> = {},
> = Path extends `/${infer Rest}` ? _CreateClient<Rest, Property>
  : Path extends "" ? _CreateClient<"index", Property>
  : _CreateClient<Path, Property>;

export type ComposeSpiceflowResponse<Response, Handle> = Handle extends (
  ...a: any[]
) => infer A ? _ComposeSpiceflowResponse<Response, Awaited<A>>
...
> = Instances extends [
...
] ? Current["_types"]["Scoped"] extends true ? MergeSpiceflowInstances<
      Rest,
      Prefix,
      Scoped,
      Singleton,
  ...
  : MergeSpiceflowInstances<
    ...
    & Routes
    & (Prefix extends `` ? Current["_routes"]
