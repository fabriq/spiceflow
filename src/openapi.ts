import { type InternalRoute, isZodSchema, Spiceflow } from "./spiceflow.ts";
...
const registerSchemaPath = ({
  ...
  if (route.path.includes(":") && !paramsSchema) {
    ...
    if (paramNames.length) {
      ...
      const paramSchemaObject: Record<string, ZodType> = {};
  ...
  const responseSchema = hooks?.response as unknown as TypeSchema;
  ...
  if (typeof responseSchema === "object") {
    ...
    if (!isStatusMap) {
      const jsonSchema = getJsonSchema(responseSchema);
      const {
        type,
        properties,
        required,
...
export const openapi = <Path extends string = "/openapi">({
  ...
  const app: Spiceflow = new Spiceflow({ name: "openapi" }).get(path, () => {
...
function getJsonSchema(schema: TypeSchema) {
  ...
  const { $schema: _$schema, ...rest } = schema as any;
  return rest as any;
}
