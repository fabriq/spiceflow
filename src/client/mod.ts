...
import type { FileList, SpiceflowClient } from "./types.ts";

export type { SpiceflowClient };
...
const isServer = typeof (globalThis as any).FileList === "undefined";

const FileList =
  (globalThis as unknown as { FileList: { new (): FileList } }).FileList;
...
const createNewFile = (v: File) =>
  isServer ? v : new Promise<File>((resolve) => {
      ...
      if (
        ...
        return Promise.resolve().then(async () => {
          ...
          if (hasFile(body)) {
            ...
            // We don't do this because we need to const the browser set the content type with the correct boundary
