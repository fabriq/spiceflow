...
Deno.test("cors middleware", async (t) => {
  ...
  await t.step(
    "GET request returns correct response and CORS headers",
    async () => {
  ...
  await t.step("OPTIONS request returns correct CORS headers", async () => {
  ...
  await t.step(
    "POST request respects CORS and returns correct response",
    async () => {
...
Deno.test("CORS headers are set when an error is thrown", async () => {
...
Deno.test("CORS headers are set for OPTIONS request when an error is thrown", async () => {
  let errorRouteCallCount = 0;
...
Deno.test("CORS headers are set when an error is thrown in middleware", async () => {
  let errorRouteCallCount = 0;
